...
all: build
	@echo "bloombox-cartridge is ready."
...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-cartridge" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-cartridge"
endif
